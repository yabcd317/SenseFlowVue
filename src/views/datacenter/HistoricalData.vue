<template>
  <div class="historical-data-layout">
    <div class="data-display-area">
      ...
      <div v-if="selectedDevices?.length" class="devices-container">
...
const hasData = computed(() => {
  return viewMode.value === 'table' ? historyData.value.length > 0 : chartData.value.length > 0
})
...
const handleDevicesUpdate = (devices) => {
  ...
  if (selectedDevices.value.length > 0) {
    fetchDeviceFactors(selectedDevices.value[0].id)
  } else {
    deviceFactors.value = []
    selectedFactors.value = []
    checkAll.value = false
...
const handleViewModeChange = (mode) => {
  if (!selectedDevices.value.length || !selectedFactors.value.length || !date.value) {
    return
  }
...
const renderChart = () => {
  ...
  const series = Object.keys(factorGroups).map(factorId => {
    ...
    const seriesData = allTimes.map(time => valueMap[time] ?? null)
...
const groupedData = computed(() => {
  ...
  const timePoints = [...new Set(historyData.value.map(item => item.recordTimeStr))].sort()
  ...
  return timePoints.map(time => {
    ...
    uniqueFactors.forEach(factor => {
      const data = dataMap[time]?.[factor]
